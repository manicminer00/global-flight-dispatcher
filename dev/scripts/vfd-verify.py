...
Run:  python dev/scripts/vfd-verify.py
  or double-click verify-vfd.bat (Windows)

Verification state is stored in dev/scripts/verification-manifest.json.
...
# Allow running as script from repo root or dev/scripts/
...
from vfd_verify_lib import (  # noqa: E402
    ...
    run_assignment_dispatch_checks,
    run_subprocess,
    save_manifest,
    uses_mission_assignments,
    utc_now,
)
...
def verify_fleet(manifest: dict, *, force: bool, stamp: bool) -> bool:
    ...
    if uses_mission_assignments():
        ...
        print("  • Mission eligibility (legacy audit-fleet-missions.py)")
    print()
    ...
    if assignment_mode:
        print("Running assignment-mode dispatch probes (authoritative)...")
        assign_errors, assign_warnings, assign_log = run_assignment_dispatch_checks()
        if assign_log:
            print(assign_log)
        print()

    print("Running fleet <-> mission reference audit...")
    code, out = run_subprocess("audit-fleet-missions.py")
    print(out)
    mission_errors = [ln.strip() for ln in out.splitlines() if "[ERROR]" in ln]
    mission_warnings = [] if assignment_mode else [ln.strip() for ln in out.splitlines() if "[WARN]" in ln]
    ...
    errors = spec_errors + mission_errors + assign_errors
    warnings = spec_warnings + mission_warnings + assign_warnings
    if force or len(need_types) >= total:
        ok = code == 0 and not errors and not assign_errors
    else:
        ok = not errors
...
def pre_deploy_run(manifest: dict, *, force: bool, stamp: bool, dedupe: bool) -> bool:
    ...
    ok = True
    if uses_mission_assignments():
        print_header("Assignment dispatch probes (always run)")
        assign_errors, assign_warnings, assign_log = run_assignment_dispatch_checks()
        if assign_log:
            print(assign_log)
        ok = print_results(assign_errors, assign_warnings) and ok
    ...
    if ok:
        ...
        cur_v = json.load(open(version_path, encoding="utf-8-sig")).get("version", "?")
        print(f"Current version: {cur_v}")
        choice = input("Bump version? patch (default) / minor / major / skip: ").strip().lower()
        if choice in ("", "p", "patch"):
            part = "patch"
        elif choice in ("m", "minor"):
            part = "minor"
        elif choice in ("major",):
            part = "major"
        else:
            part = None
        ...
        if part:
            ...
            print(f"\nVersion bumped  {cur_v}  →  {new_v}  (loader.js + version.json updated)")
            print("\nWhen ready, run in terminal:")
            print(f'  git add .')
            print(f'  git commit -m "Release v{new_v}"')
            print(f'  git push')
            print(f'  git push live main   ← only when tested and ready to go live')
        else:
