...
When mission-assignments.json is present, legacy tag/class eligibility is reference-only
(FLEET-MISSION-REFERENCE.md). Predeploy failures use Node dispatch probes instead.

Without assignments, mirrors dispatch-engine.js legacy rules.

Run: python dev/scripts/audit-fleet-missions.py
...
LONG_HAUL_EXCLUSIVE = {6, 35, 36, 37, 38}
LONG_HAUL_ALLOWED = {6, 14, 16, 18, 19, 24, 25, 26, 29, 32, 33, 35, 36, 37, 38}
# Matches dispatch-engine.js isMilitaryTroopPassengerMission (troop/passenger on logistics/heli missions).
TROOP_PAX_TYPES = {24, 30, 31}
...
def load_mission_assignments() -> dict | None:
    if not os.path.isfile(MISSION_ASSIGNMENTS_PATH):
        return None
    try:
        data = json.load(open(MISSION_ASSIGNMENTS_PATH, encoding="utf-8"))
...
def uses_mission_assignments() -> bool:
    data = load_mission_assignments()
    return bool(data and data.get("assignments"))


def get_assigned_img_ids(ac_type: str) -> list[int] | None:
    data = load_mission_assignments()
    if not data:
        return None
    ids = data["assignments"].get(ac_type)
    if ids is None:
        return None
...
def audit_tag_conventions(ac_type, spec):
    ...
    # Dual-role military airlifters need PAX tag for troop assignment on T24/T30/T31
    if (
        max_pax > 0
...
def audit_policy(ac_type, spec, eligible):
    if uses_mission_assignments():
        assigned = get_assigned_img_ids(ac_type)
        if assigned is not None and len(assigned) == 0:
            errors.append(
                f"{ac_type} ({spec.get('name', '')}): zero assigned imgIds in mission-assignments.json"
            )
        return
...
def main():
    ...
    if assignment_mode:
        n = len(load_mission_assignments()["assignments"])
        print(
            f"Assignment mode: mission-assignments.json is law ({n} aircraft).\n"
            "Legacy tag eligibility below is reference-only; predeploy uses Node dispatch probes.\n"
        )
    ...
    ref_path = os.path.join(SCRIPTS_DIR, "FLEET-MISSION-REFERENCE.md")
    ...
    print(f"\nReference written: dev/scripts/FLEET-MISSION-REFERENCE.md")
