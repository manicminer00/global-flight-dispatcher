...
Verification state lives in dev/scripts/verification-manifest.json so runtime
...
SCRIPTS_DIR = os.path.join(BASE, "dev", "scripts")
MANIFEST_PATH = os.path.join(SCRIPTS_DIR, "verification-manifest.json")

VALID_RWY = frozenset({"GA", "TURBO", "BIZ JET", "JET", "HELI", "GLIDER"})

# Non-standard identifiers confirmed valid in the Asobo DB (MSFS sim / local codes).
# These bypass the 3-5 character ICAO length rule — add new ones here when confirmed.
KNOWN_CUSTOM_ASOBO_ICAO = frozenset({"LF0429"})  # French ULM local code, Sainte-Croix-du-Verdon

# Non-standard identifiers confirmed valid in the thirdparty DB.
...
def audit_single_airport(
    ap: dict[str, Any], *, idx: int = 0, asobo_db: bool = False
    ...
    # Asobo DB uses MSFS custom identifiers which may be 5 characters (e.g. "EDOPM", "RJBET").
    # Third-party DB uses standard real-world ICAOs (3–4 chars only), except for a small set of
    # confirmed non-standard identifiers listed in KNOWN_CUSTOM_THIRDPARTY_ICAO.
    max_icao_len = 5 if asobo_db else 4
    known_custom = KNOWN_CUSTOM_ASOBO_ICAO if asobo_db else KNOWN_CUSTOM_THIRDPARTY_ICAO
    if icao not in known_custom:
        if not icao or len(icao) < 3 or len(icao) > max_icao_len:
            suffix = "3–5 characters (Asobo DB)" if asobo_db else "3–4 characters"
            errors.append(f"{label}: invalid ICAO (need {suffix})")
...
def audit_airport_db(
    ...
    *,
    ...
    is_asobo = db_file in MSFS_ICAO_PRIORITY_DATABASES
    for idx, ap in enumerate(targets, start=1):
        e, w = audit_single_airport(ap, idx=idx, asobo_db=is_asobo)
        errors.extend(e)
        warnings.extend(w)
    return errors, warnings
...
def run_node_script(script: str) -> tuple[int, str]:
    proc = subprocess.run(
        ["node", os.path.join(SCRIPTS_DIR, script)],
        capture_output=True,
        text=True,
        cwd=BASE,
    )
...
MISSION_ASSIGNMENTS_PATH = os.path.join(BASE, "mission-assignments.json")
MISSION_ASSIGNMENTS_EMBED_PATH = os.path.join(BASE, "mission-assignments-data.js")


def load_mission_assignments_data() -> dict | None:
    if not os.path.isfile(MISSION_ASSIGNMENTS_PATH):
        return None
    try:
        with open(MISSION_ASSIGNMENTS_PATH, encoding="utf-8") as fh:
            data = json.load(fh)
        if data.get("assignments") and isinstance(data["assignments"], dict):
            return data
...
def uses_mission_assignments() -> bool:
    return load_mission_assignments_data() is not None


def audit_assignment_embed_sync() -> list[str]:
    """Ensure mission-assignments-data.js embed matches mission-assignments.json."""
    issues: list[str] = []
    data = load_mission_assignments_data()
    if not data or not os.path.isfile(MISSION_ASSIGNMENTS_EMBED_PATH):
        return issues
    text = open(MISSION_ASSIGNMENTS_EMBED_PATH, encoding="utf-8").read()
    m = re.search(r"MISSION_ASSIGNMENTS_EMBED\s*=\s*(\{[\s\S]*\});", text)
    if not m:
        issues.append("mission-assignments-data.js: MISSION_ASSIGNMENTS_EMBED not found")
        return issues
    try:
        embed = json.loads(m.group(1))
    except json.JSONDecodeError:
        issues.append("mission-assignments-data.js: MISSION_ASSIGNMENTS_EMBED is not valid JSON")
        return issues
    json_assign = data.get("assignments") or {}
    embed_assign = embed.get("assignments") or {}
    all_types = set(json_assign) | set(embed_assign)
    for ac_type in sorted(all_types):
        a = sorted(json_assign.get(ac_type) or [])
        b = sorted(embed_assign.get(ac_type) or [])
        if a != b:
            issues.append(f"Assignment sync mismatch for {ac_type}: json vs mission-assignments-data.js")
    return issues


def run_assignment_dispatch_checks() -> tuple[list[str], list[str], str]:
    """Authoritative dispatch checks when mission-assignments.json is present."""
    global _assignment_dispatch_cache
    if _assignment_dispatch_cache is not None:
        errors, warnings, logs = _assignment_dispatch_cache
        return errors, warnings, logs + "\n(cached — already run this session)"

    errors: list[str] = []
    warnings: list[str] = []
    logs: list[str] = []

    if not uses_mission_assignments():
        logs.append("SKIP assignment dispatch probes (mission-assignments.json not loaded)")
        _assignment_dispatch_cache = (errors, warnings, "\n".join(logs))
        return errors, warnings, _assignment_dispatch_cache[2]

    sync_issues = audit_assignment_embed_sync()
    errors.extend(sync_issues)
    if not sync_issues:
        logs.append("OK mission-assignments.json <-> mission-assignments-data.js")

    loader = open(os.path.join(BASE, "loader.js"), encoding="utf-8").read()
    if "mission-assignments-data.js" not in loader:
        errors.append("loader.js does not load mission-assignments-data.js")
    if "initMissionAssignments" not in loader:
        errors.append("loader.js does not call initMissionAssignments()")

    for script, label in (
        ("validate-mission-assignments.mjs", "validate-mission-assignments"),
        ("dispatch-fleet-smoke.mjs", "dispatch-fleet-smoke"),
    ):
        code, out = run_node_script(script)
        logs.append(f"--- {script} ---\n{out.strip()}")
        if code != 0:
            errors.append(f"{label}: failed (exit {code})")
            for line in out.splitlines():
                stripped = line.strip()
                if stripped.startswith("✗") or "FAIL" in stripped or "error" in stripped.lower():
                    if stripped not in errors:
                        errors.append(stripped)

    log_text = "\n".join(logs)
    _assignment_dispatch_cache = (errors, warnings, log_text)
    return errors, warnings, log_text


_assignment_dispatch_cache: tuple[list[str], list[str], str] | None = None
...
def audit_fleet_specs(
    ...
    for ac_type in targets:
        ...
        for key in required:
            if key == "tags":
                if "tags" not in spec or not isinstance(spec.get("tags"), list):
                    errors.append(f"{label}: missing tags field")
        ...
        if ac_class != "GLIDER" and isinstance(fuel, (int, float)) and fuel <= 0:
            errors.append(f"{label}: fuelPerNm must be > 0 (exempt for GLIDER class)")
...
def audit_dispatch_code() -> tuple[list[str], list[str], str]:
    ...
    assignment_mode = uses_mission_assignments()
    if code != 0:
        for line in out.splitlines():
            if "[ERROR]" in line:
    ...
    if assignment_mode:
        for line in out.splitlines():
            if "[WARN]" in line:
                logs.append(f"(legacy advisory, ignored in assignment mode) {line.strip()}")
    else:
        for line in out.splitlines():
            if "[WARN]" in line:
    ...
    if assignment_mode:
        assign_errors, assign_warnings, assign_log = run_assignment_dispatch_checks()
        errors.extend(assign_errors)
        warnings.extend(assign_warnings)
        if assign_log:
            logs.append(assign_log)
    else:
        logs.append(
            "NOTE: mission-assignments.json not found — dispatch uses legacy tag rules at runtime"
        )
        warnings.append(
            "mission-assignments.json missing: browser dispatch will fall back to legacy tag rules"
        )
    ...
    # loader.js references airport files by filename, not variable name.
    for expected_file in ("airports-asobo-db.js", "airports-thirdparty-db.js"):
        if expected_file not in loader:
            errors.append(f"loader.js: expected airport database file '{expected_file}' not referenced")
    for legacy_var in LEGACY_AIRPORT_VAR_NAMES:
    ...
    if types and (max(types) > 39 or min(types) < 1):
        warnings.append(f"Mission types span {min(types)}–{max(types)} (expected ~1–39)")
