...
// Type 24 omitted — tagging almost the entire militaryTransit pool as long-haul-only left heavy
// airlifters (A400, C-130) with a single short-haul briefing (imgId 203).
...
const missionMatrix = [
    ...
    { type: 14, name: "Scheduled Commercial Service", pool: "commercial", weight: 15, allowedClasses: ["JET", "TURBO"] },
    { type: 15, name: "Regional Commuter Pulse", pool: "commercial-regional", weight: 15, allowedClasses: ["TURBO", "JET"] },
    { type: 16, name: "Executive VIP Charter", pool: "executive", weight: 15, allowedClasses: ["BIZ JET", "TURBO", "HELI"], minPaxSeats: 2, minPaxSeatsAppliesTo: ["HELI"], excludedAircraft: SMALL_GA_TRAINERS.concat(MIL_AIRLIFTER_EXCLUDED_HEAVY_FREIGHT) },
    { type: 17, name: "Light Freight Operations", pool: "lightFreight", weight: 15, allowedClasses: ["GA", "TURBO", "BIZ JET", "JET"], maxMTOW: 5000, maxMTOWAppliesTo: ["GA", "TURBO"], excludedAircraft: MAINLINE_JET_FREIGHTERS },
    ...
    { type: 19, name: "Lifeguard & Medical Relay", pool: "medical", weight: 15, allowedClasses: ["GA", "TURBO", "HELI", "BIZ JET"] },
    { type: 20, name: "Aerial Survey & Inspection", pool: "surveyServices", weight: 15, allowedClasses: ["GA", "HELI"], excludedAircraft: ["C700"], maxMTOW: 8000 },
    ...
    { type: 25, name: "Vintage & Heritage Flight", pool: "vintageOps", weight: 15, allowedClasses: ["WARBIRD", "GA", "TURBO", "JET"] },
    { type: 26, name: "Classic Airliner Charter", pool: "vintageAirliner", weight: 40, allowedAircraft: ["DC6B"], excludedAircraft: ["DC6A"] },
    { type: 27, name: "Atmospheric & Research Services", pool: "highAltServices", weight: 15, allowedClasses: ["TURBO", "JET"], excludedAircraft: ["C700", "STAR", "P180"] },
    { type: 28, name: "Passenger & Air Taxi", pool: "lightPax", weight: 15, allowedClasses: ["GA", "TURBO", "BIZ JET", "HELI"], maxMTOW: 6000, maxMTOWAppliesTo: ["GA", "TURBO", "HELI"], excludedAircraft: SMALL_GA_TRAINERS },
    { type: 29, name: "Military Cargo Transport", pool: "heavyFreight-MIL", weight: 15, allowedClasses: ["JET", "TURBO", "HELI"], minCargo: 4000, militaryOnly: true, excludedAircraft: ["VULC"] },
    ...
    { type: 32, name: "Strategic Reconnaissance", pool: "reconnaissance-MIL", weight: 20, allowedClasses: ["JET", "WARBIRD", "TURBO"], militaryOnly: true },
    ...
    { type: 35, name: "Intercontinental Scheduled Service", pool: "longHaulOps", weight: 15, allowedClasses: ["JET"] },
    { type: 36, name: "Intercontinental Heavy Freight", pool: "longHaulFreight", weight: 15, allowedClasses: ["JET", "TURBO"], minCargo: 2000, excludedAircraft: ["VULC"].concat(MIL_AIRLIFTER_EXCLUDED_HEAVY_FREIGHT) },
    { type: 37, name: "Intercontinental Executive Charter", pool: "longHaulExecutive", weight: 15, allowedClasses: ["BIZ JET"] },
    { type: 38, name: "Intercontinental Military Cargo Transport", pool: "longHaulFreight-MIL", weight: 15, militaryOnly: true, allowedClasses: ["JET", "TURBO"], minCargo: 2000, excludedAircraft: ["VULC"] },
...
const scenarioDB = {
    uniqueMissions: [
		...
		{ imgId: 10,  missionType: 10, payload: "Albatross Patrol Run", instruction: "Flying a patrol route. Keep your eyes outside the cockpit, maintain visual contact with the terrain where possible, and execute smooth turns.", allowedAircraft: ["U16"] },
        ...
        { imgId: 18, payload: "transporting tourists to their destination", instruction: "The cabin is packed with people heading for their holidays. The schedule is tight, and the passengers are excited to start their holiday. Hit your block times." },
        ...
        { imgId: 23, payload: "operating the airline's most popular route", instruction: "Execute your procedures flawlessly. Expect quick turnaround today upon arrival at the destination gate." },
        { imgId: 24, payload: "operating a scheduled commercial sector", instruction: "Time is critical. Request priority handling from ATC on the approach and arrange for an immediate gate offload upon block-in." },
        { imgId: 25, payload: "operating a standard commercial sector", instruction: "Standard line operations. Execute normal company procedures and prioritize schedule adherence." },
        { imgId: 26, payload: "shuttling morning business travelers from hub to hub", instruction: "Keep approach speeds up as requested by ATC to smoothly integrate with mainline jet traffic before utilizing your primary braking profile on rollout." },
        ...
        { imgId: 29, payload: "flying a group of exchange students to their host country", instruction: "The cabin is buzzing with excited students traveling to meet their host families. Ensure a standard, safe operational profile.", weight: 3 },
    ...
    'commercial-regional': [
        ...
        { imgId: 34, payload: "operating successive hops with short ground turnarounds", instruction: "For airframes equipped with a Hotel Mode or equivalent auxiliary system, utilize it on the ground to maintain climate controls without external power." },
	...
	executive: [
        { imgId: 45, payload: "accommodating {name} on a private schedule", instruction: "Our Passenger has requested complete quiet on this trip. Focus on a smooth climb and descent profile.", weight: 15, preferredAircraft: CELEBRITY_BIZ_JET_PREFERRED, allowedClasses: ["BIZ JET", "TURBO", "HELI"] },
        { imgId: 46, payload: "chartering {athlete} to an upcoming fixture", instruction: "Your passenger requires absolute privacy and a smooth profile to ensure proper rest before the event.", weight: 15, preferredAircraft: CELEBRITY_BIZ_JET_PREFERRED },
        ...
        { imgId: 54, payload: "transporting a secure witness", instruction: "Standard point-to-point transit with heavy discretion. Park at the designated FBO away from the main terminal.", allowedAircraft: ["C208", "STAR", "M600", "TBM8", "B60T", "BE20", "PC12", "H145", "H14M"], allowedClasses: ["BIZ JET"], weight: 3 },
        ...
        { imgId: 58, payload: "chartering a high-profile music icon", instruction: "{musician} wants to go shopping. Keep the pressurization comfortable and the bank angles shallow so they don't spill their champagne.", weight: 12, preferredAircraft: CELEBRITY_BIZ_JET_PREFERRED },
        ...
        { imgId: 61, payload: "flying {musician} to their next stadium gig", instruction: "The artist and their core management team need uninterrupted rest. Climb above the weather quickly, avoid turbulence, and keep the cabin pressure stable so they are ready for the stage.", weight: 12, preferredAircraft: CELEBRITY_BIZ_JET_PREFERRED },
        { imgId: 62, payload: "flying {name} to meet a spiritual guru", instruction: "The media is actively tracking this tail number. Park at a discrete FBO upon arrival and maintain absolute confidentiality on the radios.", weight: 15, preferredAircraft: CELEBRITY_BIZ_JET_PREFERRED, excludedAircraft: SMALL_GA_TRAINERS },
        { imgId: 63, payload: "flying {athlete} to a championship fixture", instruction: "The star athlete needs to arrive rested and on time. Request priority handling where available and keep the ride butter-smooth.", weight: 15, preferredAircraft: CELEBRITY_BIZ_JET_PREFERRED },
        { imgId: 64, payload: "flying {name} to a film premiere", instruction: "The press is watching every movement. Maintain discretion on frequency, deliver a flawless approach, and be ready for a rapid FBO turnaround.", weight: 15, preferredAircraft: CELEBRITY_BIZ_JET_PREFERRED },
        { imgId: 65, payload: "chartering {team} executives to an away fixture", instruction: "Club leadership and senior staff are travelling under tight media scrutiny. Keep the profile smooth and hit the scheduled block time.", weight: 12, preferredAircraft: CELEBRITY_BIZ_JET_PREFERRED },
        ...
        { imgId: 67, payload: "transporting a VIP to an exclusive private gala", instruction: "The client has chartered this jet for maximum comfort. The bar is fully stocked. Ensure a butter-smooth landing to keep the client in high spirits before they arrive at the event.", weight: 15, preferredAircraft: CELEBRITY_BIZ_JET_PREFERRED },
        { imgId: 68, payload: "rushing a tech CEO to a hostile board meeting", instruction: "The company's stock is plummeting and the CEO needs to be on the ground immediately to handle the fallout. Expedite all clearances and do not keep them waiting.", allowedClasses: ["BIZ JET", "TURBO", "HELI"], excludedAircraft: ["UH1", "H14M", "H65M"] },
    ],
	medical: [
        ...
        { imgId: 71, payload: "airlifting a patient from a rural strip", instruction: "Possible operations into an uncontrolled field. Verify runway conditions before committing to the approach." }
    ],
	lightFreight: [
        ...
        { imgId: 79, payload: "expediting priority medical freight", instruction: "Temperature-controlled medical cargo must reach a regional hospital without delay. Request priority handling from ATC and keep pressurization stable." },
        { imgId: 80, payload: "transporting {med_cargo}", instruction: "Cabin temperature must be strictly regulated. Avoid rapid altitude changes to prevent pressurization spikes." },
        { imgId: 81, payload: "expediting {med_cargo}", instruction: "A regional clinic requires these items for an impending procedure. Minimize block time." },
        { imgId: 82, payload: "hauling locally farmed produce from a rural strip", instruction: "The farmer has finished loading fresh produce straight from the field. Depart promptly while the cargo is still cold and dry." },
        { imgId: 83, payload: "moving regional express packages overnight", instruction: "Night freight operations. Hit your slot times to keep the logistics network flowing.", excludedAircraft: MAINLINE_JET_FREIGHTERS },
        ...
        { imgId: 90, payload: "relocating the airframe to a certified avionics shop", instruction: "The owner has reported problems with the GPS. Use good piloting skills and keep your eyes outside the cockpit." },
        ...
        { imgId: 94, payload: "connecting rural farm airstrips with mail and supplies", instruction: "A classic contract hopping short grass strips before sunset. Weight is strictly calculated at each stop. Keep your pattern work crisp and your landings soft." },
    ...
    regionalFreight: [
        ...
        { imgId: 99, payload: "expediting priority medical freight", instruction: "Temperature-controlled medical cargo must reach a regional hospital without delay. Your regional freighter is the fastest link in this chain - request priority handling from ATC." },
	...
	heavyFreight: [
        ...
        { imgId: 110, payload: "ferrying {cargo_type} and heavy oilfield equipment", instruction: "This payload is heavy and bolted straight to the floor rails. You'll be flying slower today. Watch your fuel burn and don't get behind the power curve on final." },
        ...
        { imgId: 114, payload: "moving priority overnight parcels", instruction: "Time-critical sort facility run. Request direct routing to makeup for any turnaround delays.", excludedAircraft: MIL_AIRLIFTER_EXCLUDED_OVERNIGHT_PARCELS.concat(REGIONAL_JET_FREIGHTERS) },
        ...
        { imgId: 119, payload: "transporting Formula racing chassis and paddock gear", instruction: "High-value, time-critical, and incredibly fragile. Keep the turbulence to a minimum, those front wings cost more than my house.", weight: 3, excludedAircraft: ["DC6A", "B461_MIL", "B462_MIL"].concat(MIL_AIRLIFTER_EXCLUDED_HEAVY_FREIGHT) },
    ...
    surveyServices: [
        ...
        { imgId: 125, payload: "surveying local power and pipeline grids", instruction: "Maintain minimums along the designated inspection corridor. Keep a sterile cockpit during the survey." },
        ...
        { imgId: 132, payload: "flying researchers over a {sci_fi}", instruction: "Map the area from above. Do your best to ignore the bizarre static interference bleeding through the comms.", weight: 3 },
        { imgId: 133, payload: "assisting authorities in a visual search for a missing vehicle", instruction: "Fly over the last known coordinates at a low altitude. Keep your airspeed at best endurance and keep your eyes peeled for improvised distress signals.", weight: 3, excludedAircraft: ["BE60", "STAR"] },
    ],
    lightPax: [
        ...
        { imgId: 137, payload: "providing critical passenger links to a community", instruction: "This community depends on your light aircraft connection. Adhere tightly to the schedule - many passengers have tight onward connections at the destination hub." },
        ...
        { imgId: 140, payload: "flying a romantic, scenic twilight flight", instruction: "Keep the engine RPM smooth, avoid steep banks, and maintain a gentle flight profile to ensure the moment goes off without a hitch.", weight: 1 },
        { imgId: 141, payload: "rushing a stranded businessman to a regional hub", instruction: "Your passenger missed their commercial connection and booked the last available seat with you. Can you save the day?", weight: 2 },
        { imgId: 142, payload: "bringing passengers home to their families", instruction: "Your service is the only link between these communities today. Plan your fuel carefully, watch for approach winds, and grease the landing." },
        { imgId: 143, payload: "flying a honeymooning couple to a remote retreat", instruction: "A newly-wed couple has chartered your light aircraft for a romantic getaway. Keep the turns gentle, find smooth air for the cruise, and make the landing as soft as they expect the holiday to be.", weight: 1 },
        { imgId: 144, payload: "transporting a regional medical examiner to an incident site", instruction: "Time-sensitive call-out. The medical examiner needs to reach the site as quickly as safely possible. Confirm runway conditions before committing to the approach.", weight: 3 },
        { imgId: 145, payload: "delivering urgent freight to a private airstrip", instruction: "A rural estate has placed an urgent order for essential materials. Your light aircraft is the only option for this short-strip destination. Watch your crosswind component on final." },
        { imgId: 146, payload: "repositioning an aircraft for its new owner", instruction: "The deal is done and the keys have been handed over. Ferry this light aircraft from the broker's field to its new home base. Monitor engine temps carefully on this unfamiliar airframe." }
    ],
    highAltServices: [
    ...
    vintageOps: [
        { imgId: 149, payload: "performing a historic flight demonstration", instruction: "The aircraft is requested for a static display and a brief aerial routine. Ensure all fluid systems are checked before departure.", allowedClasses: ["WARBIRD", "JET"], excludedAircraft: ["DC6A"], weight: 3 },
        ...
        { imgId: 152, payload: "acting as a chase plane for a historic squadron", instruction: "Maintain visual separation and monitor engine temperatures during extended formation power settings.", allowedClasses: ["GA"] },
        ...
        { imgId: 156, payload: "ferrying to a regional aviation festival", instruction: "You are expected at the event staging grounds before sunset. Keep your cruise power settings conservative to preserve engine life. Expect crowds watching your arrival." },
    ...
    'helicopterOps-CIV': [
        ...
        { imgId: 171, payload: "extracting a trauma victim from a major roadway", instruction: "First responders have secured a landing zone. Proceed immediately." },
        { imgId: 172, payload: "locating a stranded group of travellers", instruction: "Execute search patterns over the last known coordinates. Be prepared for hoisting operations." },
        ...
        { imgId: 175, payload: "positioning industrial equipment", instruction: "Precision hover required. Monitor engine limits closely during the heavy lift phase." },
        ...
        { imgId: 180, payload: "moving heavy pipe segments into rugged terrain", instruction: "You will be conducting precision hovering operations with external loads. Keep a close eye on engine torque and rotor RPM limits while maneuvering the segments into place.", minCargo: 201 },
        ...
        { imgId: 183, payload: "airlifting an unidentifiable metallic pillar", instruction: "A smooth, impossibly heavy metallic pillar was discovered deep in a slot canyon. Ground crews are reporting sudden temperature drops and severe compass spin near the object.", minCargo: 201, weight: 1 },
    ],
	// MILITARY
	'heavyFreight-MIL': [
    ...
    'helicopterOps-MIL': [
        { imgId: 191, payload: "deploying a specialized ground team", instruction: "Approach the LZ utilizing terrain masking. Ensure a stabilized hover for deployment.", heliOps: true },
        ...
        { imgId: 195, payload: "delivering essential provisions to an observation post", instruction: "Payload includes field rations and replacement radio batteries. Approach the LZ carefully, watching for unpredictable wind shear.", heliOps: true },
        { imgId: 196, payload: "transporting regional commanders to a joint briefing", instruction: "Provide a comfortable transit. Security protocols dictate a direct routing with no unnecessary deviations.", staffShuttle: true }
    ],
    'militaryTransit-MIL': [
        ...
        { imgId: 198, payload: "moving unmarked secure containers", instruction: "Flight plan filed under discrete routing. Maintain standard communication but proceed directly to the destination." },
        { imgId: 199, payload: "relocating contracted defense personnel", instruction: "Standard military charter. Maintain schedule and follow standard heavy jet procedures.", excludedAircraft: ["VULC"] },
        ...
        { imgId: 201, payload: "executing a peacetime navigation exercise", instruction: "Focus on precise fuel management and exact waypoint timings. Maintain standard ATC communications where required." },
        { imgId: 202, payload: "relocating staff personnel under tactical escort", instruction: "Low-profile transit. Direct routing required with strict adherence to assigned flight levels.", excludedAircraft: ["VULC"] },
        ...
        { imgId: 205, payload: "providing high-speed search and rescue support", instruction: "Scan along the flight path for emergency beacons or visual distress signals. Relay coordinates to the extraction teams and maintain your assigned low-level search profile." },
        { imgId: 206, payload: "patrolling the designated vector for surface anomalies", instruction: "Maintain visual contact with the surface and report any unregistered traffic to sector command." },
        { imgId: 207, payload: "providing a localized UHF/VHF relay for ground exercises", instruction: "Establish a stable holding pattern over the sector. Monitor fuel consumption during this extended endurance task." }
    ],
    'tacticalJet-MIL': [
    ...
    'reconnaissance-MIL': [
        { imgId: 220, missionType: 32, payload: "conducting aerial refuelling approach practice", instruction: "Fly the assigned route from departure to destination. Maintain the briefed approach geometry, airspeed, and altitude tolerances throughout the transit for tanker rendezvous training.", allowedAircraft: STRATEGIC_RECON_AIRCRAFT, excludedAircraft: ["SPIT", "BF109", "F6F", "P38", "FW08", "A6M5"] },
        { imgId: 221, missionType: 32, payload: "conducting a transit reconnaissance sortie", instruction: "Fly the assigned route from departure to destination. Maintain the filed altitude and airspeed profile so onboard sensors can collect continuous imagery along the corridor.", allowedAircraft: STRATEGIC_RECON_AIRCRAFT },
        { imgId: 222, missionType: 32, payload: "mapping the assigned transit corridor", instruction: "Execute a direct routing from departure to destination. Maintain steady track and altitude so mapping systems can build a complete mosaic of the route.", allowedAircraft: STRATEGIC_RECON_AIRCRAFT, preferredAircraft: VULCAN_RECON_PREFERRED },
        { imgId: 223, missionType: 32, payload: "photographing designated areas along the route", instruction: "Transit from departure to destination along the filed routing. Maintain precise heading and timing so ground analysts can correlate imagery with the mission timeline.", allowedAircraft: STRATEGIC_RECON_AIRCRAFT, preferredAircraft: VULCAN_RECON_PREFERRED },
        { imgId: 249, missionType: 32, payload: "conducting a two-ship formation proficiency transit", instruction: "Fly the filed intercontinental route from departure to destination. Ensure wingman separation remains within specified parameters throughout the extended climb and oceanic cruise.", allowedAircraft: STRATEGIC_RECON_AIRCRAFT, preferredAircraft: VULCAN_RECON_PREFERRED },
        { imgId: 250, missionType: 32, payload: "conducting aerial refuelling approach practice", instruction: "Fly the assigned long-range route from departure to destination. Maintain the briefed approach geometry, airspeed, and altitude tolerances throughout the oceanic transit for tanker rendezvous training.", allowedAircraft: STRATEGIC_RECON_AIRCRAFT, excludedAircraft: ["SPIT", "BF109", "F6F", "FW08", "P38"] }
    ],
    gliderOps: [
        ...
        { imgId: 227, payload: "conducting a dual instructional sortie from {dep_field}", instruction: "Your student is aboard for handling practice and lookout drills. Demonstrate energy management, pre-landing checks, and disciplined circuit work. Talk through how you read the sky - growing Cu for strong thermals, cloud streets for efficient transit, and surface wind cues for positioning. Keep the ride smooth and verbalize every decision." },
        { imgId: 228, payload: "ferrying under power from {dep_field} to {dest_field}", instruction: "The club needs this airframe repositioned. Self-launch or sustain as required, climb to a safe transit altitude, and shut down to soar whenever lift allows. If you convert to soaring, treat developing cumulus as signposts for lift. Conserve battery or fuel for a go-around reserve at {dest_field}." },
    ...
    longHaulOps: [
        { imgId: 230, payload: "operating a scheduled intercontinental passenger sector", instruction: "You are cleared on a long-range link between major international airports. Brief oceanic or remote-area contingencies, monitor fuel against alternates, and expect heavy jet sequencing on arrival." },
        { imgId: 231, payload: "ferrying hub-connecting traffic on an ultra-long sector", instruction: "The cabin is full of passengers transiting between global hubs. Maintain your assigned cruise level, adhere to the filed track, and keep the ride smooth through the cruise phase." },
        { imgId: 232, payload: "operating an overnight red-eye across multiple time zones", instruction: "Cabin lighting is dimmed and most passengers are asleep. Manage fatigue, monitor NAT or PAC tracks if assigned, and plan a stabilized dawn arrival." },
        { imgId: 233, payload: "positioning a mainline jet on a flagship international route", instruction: "This is one of the airline's longest scheduled sectors. Verify fuel load, ETOPS or long-range planning as applicable, and coordinate with oceanic control before coast-out." },
        { imgId: 234, payload: "recovering stranded passengers on a long-range repatriation sector", instruction: "Aircraft and crew were repositioned specifically for this extended recovery flight. Passengers have been waiting days — prioritize schedule adherence and a comfortable cruise profile." },
        { imgId: 235, payload: "operating a seasonal long-haul leisure route", instruction: "Holiday traffic is heavy and block fuel is high. Listen for ATC re-routes around weather systems and manage step-climbs to optimize burn on this extended sector." }
    ],
    longHaulFreight: [
        { imgId: 236, payload: "hauling priority intercontinental air cargo", instruction: "Main-deck and belly freight must reach the destination hub on schedule. Monitor fuel closely on this extended sector and confirm offload teams are standing by." },
        { imgId: 237, payload: "ferrying time-critical logistics on a long-range freight sector", instruction: "Warehouse cut-off times are tight on both ends of this route. Request oceanic clearance promptly and keep the ride smooth to protect sensitive pallet loads." },
        { imgId: 238, payload: "repositioning a freighter for an international sort window", instruction: "You are deadheading or lightly loaded to catch the next bank at a global cargo hub. Plan step climbs and respect maximum range reserves on this extended leg." }
    ],
    longHaulExecutive: [
        { imgId: 244, payload: "ferrying {name} on a transoceanic executive charter", instruction: "Your VIP expects a quiet cabin and an on-time arrival at the FBO. Plan fuel reserves carefully on this extended sector and keep bank angles gentle through the cruise." },
        { imgId: 245, payload: "flying {athlete} to an overseas championship fixture", instruction: "The passenger needs maximum rest on this extended leg. Climb above weather early, keep the ride smooth, and plan step climbs to optimize fuel on the oceanic portion." },
        { imgId: 246, payload: "relocating {musician} on a long-range tour leg", instruction: "The schedule is tight and the passenger must rest before the next performance. Monitor NAT or remote-area contingencies and maintain a smooth cruise profile." },
        { imgId: 247, payload: "conducting a discreet intercontinental business shuttle", instruction: "Corporate leadership is aboard for a time-critical overseas meeting. Maintain discretion on frequency and deliver a stabilized arrival at the destination FBO." },
        { imgId: 248, payload: "operating a long-range medevac-capable executive repositioning", instruction: "Medical support equipment is staged aboard in case it is needed en route. Verify oxygen and electrical loads against your range plan before coast-out." }
    ],
    "longHaulFreight-MIL": [
        { imgId: 239, payload: "executing a long-range strategic airlift", instruction: "Palletized military stores must reach the forward operating base on schedule. Monitor fuel against alternates across remote segments and confirm offload security on arrival." },
        { imgId: 240, payload: "ferrying heavy tactical equipment on an intercontinental deployment leg", instruction: "Centre of gravity is critical on this extended sector. Request priority handling where available and maintain strict adherence to the filed oceanic track." },
        { imgId: 241, payload: "delivering humanitarian relief on a strategic military airbridge", instruction: "Relief pallets are bound for a forward distribution hub. Maintain schedule through remote airspace and confirm ground teams are standing by on arrival." },
        { imgId: 242, payload: "repositioning a military airlifter for an overseas exercise", instruction: "You are moving the airframe and support stores to join a multi-national deployment. Plan step climbs and respect maximum range reserves on this extended leg." },
        { imgId: 243, payload: "moving classified logistics containers on a discrete long-range transit", instruction: "Flight plan routing is filed under operational security protocols. Proceed directly to destination and prepare for a controlled ramp offload." }
    ]
};
