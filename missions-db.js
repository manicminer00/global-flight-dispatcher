const missionMatrix = [
    ...
    { type: 1, weight: 2, name: "EGLC Steep Departure", pool: "uniqueMissions", requiredDep: "EGLC", maxMTOW: 75500 },
    { type: 2, weight: 2, name: "EGNS Steep Departure", pool: "uniqueMissions", requiredDep: "EGNS", maxMTOW: 75500 },
    { type: 3, weight: 2, name: "Santos Dumont Departure", pool: "uniqueMissions", requiredDep: "SBRJ", maxMTOW: 75500 },
    { type: 4, weight: 2, name: "Comanche Cross Country", pool: "uniqueMissions" },
	{ type: 5, weight: 2, name: "High-Altitude Transit", pool: "uniqueMissions", minAlt: 10000, rules: "IFR" },
    { type: 7, weight: 2,  name: "Whisperjet Operations", pool: "uniqueMissions" },
    { type: 8, weight: 2,  name: "Geological Research", pool: "uniqueMissions" },
    { type: 9, weight: 2,  name: "Outpost Supply Run", pool: "uniqueMissions" },
    { type: 10, weight: 2, name: "Albatross Patrol", pool: "uniqueMissions" },
    { type: 11, weight: 2, name: "Survey Team Transfer", pool: "uniqueMissions" },
    { type: 12, weight: 2, name: "Bulk Supply Drop", pool: "uniqueMissions" },
    { type: 13, weight: 2, name: "Heritage SAR Demo", pool: "uniqueMissions" },
    // DYNAMIC MASTER TEMPLATES - no image required
    { type: 14, weight: 10, name: "Commercial Service", pool: "commercial" },
    { type: 15, weight: 10, name: "Regional Commuter", pool: "commercial-regional" },
    { type: 16, weight: 10, name: "Executive Charter", pool: "executive" },
    { type: 17, weight: 10, name: "Light Freight Ops", pool: "lightFreight", maxMTOW: 5000, maxMTOWAppliesTo: ["GA", "TURBO"] },
    { type: 18, weight: 10, name: "Heavy Cargo Transport", pool: "heavyFreightMissions", minCargo: 2000 },
    { type: 19, weight: 10, name: "Medical Relay", pool: "medical" },
    { type: 20, weight: 10, name: "Aerial Survey", pool: "surveyServices", maxMTOW: 8000 },
    { type: 21, weight: 10, name: "Rotary Wing Operations", pool: "helicopterMissions" },
    { type: 23, weight: 10, name: "Tactical Sortie", pool: "tacticalJet-MIL", militaryOnly: true, tacticalOnly: true },
    { type: 24, weight: 10, name: "Military Logistics", pool: "militaryTransit-MIL", militaryOnly: true },
    { type: 25, weight: 10, name: "Heritage Flight", pool: "vintageOps" },
    { type: 26, weight: 10, name: "Classic Airliner Charter", pool: "vintageAirliner" },
    { type: 27, weight: 1, name: "Weather Ops", pool: "highAltServices" },
    { type: 28, weight: 10, name: "Air Taxi", pool: "lightPax", maxMTOW: 6000, maxMTOWAppliesTo: ["GA", "TURBO", "HELI"] },
    { type: 32, weight: 10, name: "Strategic Recon.", pool: "reconnaissance-MIL", militaryOnly: true },
    { type: 33, weight: 10, name: "Vintage Propliner Freight", pool: "vintageProplinerFreight", minCargo: 2000 },
    { type: 34, weight: 10, name: "Gliding Operations", pool: "gliderOps", rules: "VFR/Scenic" },
    { type: 39, weight: 10, name: "Regional Freight Pulse", pool: "regionalFreight" }
];

const scenarioDB = {
    uniqueMissions: [
        { imgId: 1, title: "EGLC Steep Departure", missionType: 1, instruction: "EGLC demands a disciplined departure. Fly the approved steep-climb profile, manage thrust carefully, and keep the noise footprint to a minimum." },
        { imgId: 2, title: "EGNS Steep Departure", missionType: 2, instruction: "Noise restrictions make this a precision departure. Establish the steep-climb profile promptly and remain within the published limits." },
        { imgId: 3, title: "Santos Dumont Departure", missionType: 3, instruction: "Santos Dumont leaves little room for delay. Use the available runway efficiently, establish the climbing turn after departure, and maintain close terrain awareness." },
        { imgId: 4, title: "Comanche Cross Country", missionType: 4, instruction: "You are taking part in a cross-country rally. Manage the mixture carefully and monitor the cylinder-head temperatures." },
        { imgId: 5, title: "High Altitude Transit", missionType: 5, instruction: "Take this service above the lower-level weather and settle into an efficient cruise. Monitor the oxygen system and keep the flight on schedule." },
        { imgId: 7, title: "Whisperjet Operations", missionType: 7, instruction: "This aircraft earned its reputation through quiet, efficient operation. Fly a stable descent and configure early enough for a smooth landing." },
        { imgId: 8, title: "Geological Research",   missionType: 8, instruction: "A university team is measuring changes in the surface below. Hold a steady low-level track and monitor the Albatross's engine temperatures throughout the survey." },
        { imgId: 9, title: "Outpost Supply Run",   missionType: 9, instruction: "The Albatross is carrying a full load of overdue supplies. Expect heavier handling, plan the approach carefully, and protect the cargo with a smooth arrival.", minCargoLoadFactor: 0.70 },
		{ imgId: 10, title: "Albatross Patrol",  missionType: 10, instruction: "The Albatross is scheduled for another observation patrol. Maintain a steady profile, keep a good lookout, and use smooth, coordinated turns." },
        { imgId: 11, title: "Survey Team Transfer", missionType: 11, instruction: "A survey team and its sensitive equipment are on board. Avoid abrupt manoeuvres and deliver both passengers and instruments with a gentle landing." },
        { imgId: 12, title: "Bulk Supply Drop", missionType: 12, instruction: "Essential supplies are needed at the destination. Confirm the delivery area, fly a controlled approach, and release the load cleanly.", minCargoLoadFactor: 0.70 },
        { imgId: 13, title: "Heritage SAR Demo", missionType: 13, instruction: "This demonstration honours the flying-boat crews who served on search-and-rescue duty. Maintain a vigilant lookout and fly the Albatross with measured, deliberate handling." }
    ],
        commercial: [
    ...
    'commercial-regional': [
        ...
        { imgId: 37, weight: 10, title: "Weather Recovery Sector", instruction: "Previous cancellations have left a cabin of tired passengers waiting to get home. Deliver a calm recovery flight and avoid any further disruption." },
        { imgId: 38, weight: 10, title: "Holiday Sector", instruction: "Every seat is occupied by passengers ready to begin their holiday. Keep the departure punctual and give them a comfortable start to the journey." },
	...
	executive: [
        ...
        { imgId: 46, weight: 10,title: "Wheels Up", instruction: "{athlete} needs uninterrupted rest before the next engagement. Keep the flight smooth from brake release to touchdown so they arrive refreshed" },
        { imgId: 47, weight: 10, title: "Deal Team Charter", instruction: "The passengers are negotiating a major deal in the cabin. Maintain a quiet environment and avoid interruptions unless operationally necessary." },
	...
	lightFreight: [
        ...
        { imgId: 75, weight: 6, title: "Sapling Delivery", instruction: "Young saplings are travelling under controlled conditions. Maintain stable pressure and temperature, and use a gentle descent before arrival.", minCargoLoadFactor: 0.40 },
    ...
    regionalFreight: [
        { imgId: 96, weight: 10, title: "Night Express", instruction: "The overnight network depends on every slot being met. Protect the schedule and avoid becoming the delay that holds the entire system back." },
        { imgId: 97, weight: 10, title: "Priority Parcels", instruction: "The sorting facility is working to a strict cutoff. Request efficient routing at the first sign of delay and protect the arrival window." },
        { imgId: 98, weight: 10, title: "Regional Red-Eye", instruction: "This overnight service links the regional hubs while traffic is light. Meet the assigned slots and fly a quiet, stable descent." },
        { imgId: 99, weight: 10, title: "Medical Freight Link", instruction: "Temperature-controlled medical freight is urgently required at a regional hospital. Maintain the cold chain and complete the quickest safe transfer available." },
        { imgId: 100, weight: 10, title: "Lab Samples", instruction: "{med_cargo} are travelling directly to the laboratory without an intermediate stop. Maintain the specified conditions and deliver them to the waiting medical team." },
        { imgId: 101, weight: 10, title: "Field Machinery", instruction: "Replacement machinery components are being collected for regional farms. Confirm the loading plan and ensure every item is secured before departure." },
        { imgId: 102, weight: 10, title: "Regional Restock", instruction: "This service is restocking businesses across the region. Review the landing performance carefully and use reverse thrust only as conditions require." },
        { imgId: 103, weight: 10, title: "Late Night Regional", instruction: "A quiet late-night regional sector is on the schedule. Use the lighter traffic to fly efficiently and conserve fuel without delaying the service." }
    ],
    ...
    'militaryTransit-MIL': [
        ...
        { imgId: 198, weight: 7, title: "Unmarked Container Run", instruction: "Unmarked secure containers are listed on a restricted manifest. Follow the approved flight plan and proceed directly to the designated destination handling area." },
